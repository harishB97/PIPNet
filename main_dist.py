...
def create_distributed_dataloader(original_dataloader, rank, world_size):
    dataset = original_dataloader.dataset
    new_sampler = DistributedSampler(dataset, num_replicas=world_size, rank=rank)
...
def run_pipnet(args=None):
    ...
    trainloader = create_distributed_dataloader(trainloader, rank, world_size)
    trainloader_pretraining = create_distributed_dataloader(trainloader_pretraining, rank, world_size)
    trainloader_normal = create_distributed_dataloader(trainloader_normal, rank, world_size)
    trainloader_normal_augment = create_distributed_dataloader(trainloader_normal_augment, rank, world_size)
    projectloader = create_distributed_dataloader(projectloader, rank, world_size)
    testloader = create_distributed_dataloader(testloader, rank, world_size)
    test_projectloader = create_distributed_dataloader(test_projectloader, rank, world_size)
    ...
    net = net.to(device=rank)
    # net = nn.DataParallel(net, device_ids = device_ids)    
    net = DDP(net, device_ids = [rank], find_unused_parameters=True, static_graph=False)    
