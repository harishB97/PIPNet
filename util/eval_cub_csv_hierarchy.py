...
def get_topk_cub(net, projectloader, k, epoch, device, args, node=None):    
    # Make sure the model is in evaluation mode
    net.eval()
    ...
    for i, (xs, orig_y, ys) in project_iter:
        ...
        with torch.no_grad():
            ...
            for p in range(pooled.shape[0]):
                ...
                relevant_proto_classes = torch.nonzero(classification_weights[:, p] > 0)
                ...
                if (coarse_label2name[ys.item()] in relevant_proto_class_names):
                    ...
                    leaf_descendent = label2name[orig_y.item()]
                    if p not in scores_per_prototype:
                        scores_per_prototype[p] = []
                    scores_per_prototype[p].append((i, leaf_descendent, pooled[p].item()))

                # if c_weight > 1e-5:#ignore prototypes that are not relevant to any class
                #     if p not in scores_per_prototype:
                #         scores_per_prototype[p] = []
                #     # TODO find protos descendant and add that as well
                #     scores_per_prototype[p].append((i, pooled[p].item()))
    
    proto_img_coordinates = []
    csvfolderpath = os.path.join(args.log_dir, 'node_wise_topk')
    os.makedirs(csvfolderpath, exist_ok=True)
    csvfilepath = os.path.join(csvfolderpath, str(epoch)+f'_pipnet_prototypes_cub_topk_{node.name}.csv')
    too_small = set()
    ...
    with open(csvfilepath, "w", newline='') as csvfile:
        ...
        writer = csv.writer(csvfile, delimiter=',')
        writer.writerow(["prototype", "leaf_descendent", "img name", "h_min_224", "h_max_224", "w_min_224", "w_max_224", "scores"])
        for _, prototype in protoype_iter:
            df = pd.DataFrame(scores_per_prototype[prototype], columns=['img_id', 'leaf_descendent', 'scores'])
            # TODO Get topk for each proto descendant pair
            # topk = df.nlargest(k, 'scores')
            topk_df = df.groupby(['img_id', 'leaf_descendent']).apply(lambda x: x.nlargest(k, 'scores')).reset_index(drop=True)
            for index, row in topk.iterrows():
                imgid = int(row['img_id'])
                leaf_descendent = row['img_id']
                imgname = imgs[imgid][0]
                image = transforms.Resize(size=(args.img_size, args.img_size))(Image.open(imgname))
                ...
                with torch.no_grad():
                    ...
                    proto_img_coordinates.append([prototype, leaf_descendent, imgname, h_coor_min, h_coor_max, w_coor_min, w_coor_max, row['scores']])
